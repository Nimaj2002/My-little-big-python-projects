...
class Round:
    ...
    def bidding(self):
        for i, player in enumerate(self.players):
            ...
            if (i < len(self.players) - 1):
                ...
                self.print_header()
                handle_input(
                    "All bids are made! Please ensure that all players see the header so they know what other bids they are up against!\n\n"
    ...
    def play_trick(self, ordering):
        ...
        for i, player in enumerate(ordering):
            ...
            if (len(selectable) == 0):
                selectable = [i for i in
                              range(len(self.hands[player]))]  # If nothing was playable, all cards are fair game!
            else:
                for i, card in enumerate(self.hands[player]):
    ...
    def play_round(self):
        ...
        for i in range(13):
            ...
            msg = (
                f"Player {lead_player} won the hand with {best_card.order}{best_card.suite}! Press any key to continue...")
